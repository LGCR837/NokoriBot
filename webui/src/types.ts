export const APP_NAME = 'NokoriBot';
...
// ─── NokoriBot-specific types ──────────────────────────────────────────────
// These map NokoriBot's backend responses to SL-compatible shapes.

export interface NokoriBotStatusResponse {
  bot: {
    running: boolean;
    loggedIn: boolean;
    myUid: string | null;
...
export interface NokoriBotFriend {
  uid: string;
  name: string;
  remark?: string;
}

export interface NokoriBotGroup {
  group_id: string;
  name: string;
  member_count?: number;
}
