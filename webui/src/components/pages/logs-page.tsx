...
function sanitizeLogLine(line: string): string {
  const ESC = 0x1b, BEL = 0x07, C1_CSI = 0x9b, C1_ST = 0x9c, C1_OSC = 0x9d;
  const C1_STRING_STARTERS = new Set([0x90, 0x98, 0x9e, 0x9f]);
  function skipCsi(s: string, i: number): number { for (; i < s.length; i++) { const c = s.charCodeAt(i); if (c >= 0x40 && c <= 0x7e) return i + 1; } return s.length; }
  function skipCS(s: string, i: number, bell: boolean): number { for (; i < s.length; i++) { const c = s.charCodeAt(i); if (bell && c === BEL) return i + 1; if (c === C1_ST) return i + 1; if (c === ESC && s.charCodeAt(i + 1) === 0x5c) return i + 2; } return s.length; }
  function skipEsc(s: string, i: number): number { if (i >= s.length) return s.length; const f = s.charCodeAt(i); if (f === 0x5b) return skipCsi(s, i + 1); if (f === 0x5d) return skipCS(s, i + 1, true); if (f === 0x50 || f === 0x58 || f === 0x5e || f === 0x5f) return skipCS(s, i + 1, false); let j = i; while (j < s.length) { const c = s.charCodeAt(j); if (c < 0x20 || c > 0x2f) break; j++; } const fin = s.charCodeAt(j); return fin >= 0x30 && fin <= 0x7e ? j + 1 : i; }
  let out = '';
  for (let i = 0; i < line.length;) { const code = line.charCodeAt(i);
    if (code === ESC) { i = skipEsc(line, i + 1); continue; }
    if (code === C1_CSI) { i = skipCsi(line, i + 1); continue; }
    if (code === C1_OSC || C1_STRING_STARTERS.has(code)) { i = skipCS(line, i + 1, code === C1_OSC); continue; }
    if ((code < 0x20 && code !== 0x09 && code !== 0x0a) || (code >= 0x7f && code <= 0x9f)) { i++; continue; }
    out += line[i]; i++; }
  return out;
}
